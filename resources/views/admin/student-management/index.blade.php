...
@push('styles')
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .student-search-row .flex-1.relative {
                min-width: 0;
            }
        ...
        <div class="card">
            ...
            <div class="flex flex-wrap gap-4 items-start">
                <div class="flex-1 min-w-0">
                    <label for="student_id_search" class="block text-sm font-medium text-primary mb-2">Student ID or Name
                        *</label>
                    <div class="flex flex-wrap items-center gap-3 student-search-row">
                        <div class="flex-1 relative min-w-0">
                            <input type="text" id="student_id_search" autocomplete="off"
                                placeholder="Type Student ID or Name for suggestions..."
                        ...
                        <button type="button" onclick="searchStudent()"
                            class="inline-flex items-center justify-center px-6 py-2 rounded-lg font-medium text-white transition-all hover:opacity-90 whitespace-nowrap btn-add"
                            style="background: linear-gradient(135deg, var(--cpsu-green) 0%, var(--cpsu-green-light) 100%);">
                            Search
                        </button>
                    </div>
                    <p class="text-xs text-secondary mt-1">Type to see matching Student ID or Name suggestions; then select
                        or click Search.</p>
            ...
            <form id="createAccountForm" onsubmit="createAccount(event)">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 student-password-row">
                            <input type="text" id="password" name="password" required readonly
                                class="flex-1 min-w-0 px-4 py-2 rounded-lg transition-all font-mono"
                                style="background-color: var(--card-bg); color: var(--text-primary); border: 1px solid var(--border-color);"
                                placeholder="Click Generate Password">
                            <button type="button" onclick="generatePassword()"
                                class="inline-flex items-center justify-center px-4 py-2 rounded-lg font-medium text-white transition-all hover:opacity-90"
                                style="background: linear-gradient(135deg, var(--cpsu-green) 0%, var(--cpsu-green-light) 100%);">
                                Generate Password
                            </button>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-3 student-form-actions">
                        <button type="submit"
                            class="inline-flex items-center justify-center px-6 py-2 rounded-lg font-medium text-white transition-all hover:opacity-90 btn-add"
                            style="background: linear-gradient(135deg, var(--cpsu-green) 0%, var(--cpsu-green-light) 100%);">
                            Create Account
                        </button>
                        <button type="button" onclick="resetForm()"
                            class="inline-flex items-center justify-center px-6 py-2 rounded-lg font-medium transition-all"
            ...
            <div class="overflow-x-auto table-wrap">
                <table class="min-w-full divide-y" style="border-collapse: separate; border-spacing: 0;">
                    <thead>
                        <tr>
                            <th scope="col"
                                class="px-6 py-3 text-left text-xs font-semibold text-secondary uppercase tracking-wider border-b"
                            ...
                            <tr class="hover:bg-[var(--hover-bg)] transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex flex-col items-center">
                                        <span
                                            class="text-sm font-semibold text-primary">{{ $account->password_regenerated_count ?? 0 }}</span>
                                        <span class="text-xs text-secondary">time(s)</span>
                                        @if (($account->password_regenerated_count ?? 0) > 0)
                                            <button onclick="showPasswordHistory({{ $account->id }})"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="text-secondary opacity-75">
                                        ...
                                        <p class="text-lg font-semibold text-primary mb-1">No student accounts created yet
                                        </p>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-secondary">
                            ...
                            <strong class="text-primary"
                                style="color: var(--cpsu-green);">{{ $studentAccounts->total() }}</strong> accounts
                        </div>
                        <div class="flex items-center space-x-1">
                            @if ($studentAccounts->onFirstPage())
                                <span class="px-3 py-2 rounded-lg text-sm font-medium opacity-50 cursor-not-allowed"
                                    style="background-color: var(--bg-tertiary); color: var(--text-secondary); border: 1px solid var(--border-color);">
                                    Previous
                                </span>
                            @else
                                <a href="{{ $studentAccounts->previousPageUrl() }}"
                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all hover:bg-[var(--hover-bg)]"
                                    style="background-color: var(--card-bg); color: var(--text-primary); border: 1px solid var(--border-color);">
                                    Previous
                                </a>
                            @endif
                            ...
                            @foreach ($studentAccounts->getUrlRange(max(1, $studentAccounts->currentPage() - 2), min($studentAccounts->lastPage(), $studentAccounts->currentPage() + 2)) as $page => $url)
                                @if ($page == $studentAccounts->currentPage())
                                    <span class="px-4 py-2 rounded-lg text-sm font-semibold text-white shadow-sm"
                                        style="background: linear-gradient(135deg, var(--cpsu-green) 0%, var(--cpsu-green-light) 100%);">
                                        {{ $page }}
                                    </span>
                                @else
                                    <a href="{{ $url }}"
                                        class="px-4 py-2 rounded-lg text-sm font-medium transition-all hover:bg-[var(--hover-bg)]"
                                        style="background-color: var(--card-bg); color: var(--text-primary); border: 1px solid var(--border-color);">
                                        {{ $page }}
                                    </a>
                                @endif
                            ...
                            @if ($studentAccounts->hasMorePages())
                                <a href="{{ $studentAccounts->nextPageUrl() }}"
                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all hover:bg-[var(--hover-bg)]"
                                    style="background-color: var(--card-bg); color: var(--text-primary); border: 1px solid var(--border-color);">
                                    Next
                                </a>
                ...
                <div id="passwordHistoryContent">
                    <div class="text-center py-8">
                        <div class="animate-spin rounded-full h-8 w-8 border-b-2 mx-auto"
                            style="border-color: var(--cpsu-green);"></div>
            ...
            <div class="modal-body">
                <div class="flex items-center space-x-4 mb-4">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0"
                        style="background: rgba(0, 136, 68, 0.1);">
                        <svg class="w-6 h-6" style="color: var(--cpsu-green-light);" fill="none"
                ...
                <div class="space-y-2 p-3 rounded-lg" style="background-color: var(--bg-tertiary);">
                    ...
                    <p class="text-xs text-secondary mt-2">Previous regenerations: <span
                            class="font-semibold text-primary" id="regenerateCount">0</span> time(s)</p>
                </div>
                <div class="mt-4 p-3 rounded-lg"
                    style="background-color: rgba(255, 193, 7, 0.1); border: 1px solid rgba(255, 193, 7, 0.3);">
                    <p class="text-xs" style="color: #d97706;">
                        <strong>Warning:</strong> A new password will be generated. The old password will no longer work.
                        Make sure to provide the new password to the student.
                    </p>
                </div>

                <!-- Generated Password Display (initially hidden) -->
                <div id="generatedPasswordSection" class="mt-4 p-4 rounded-lg"
                    style="display: none; background-color: rgba(0, 136, 68, 0.1); border: 1px solid rgba(0, 136, 68, 0.3);">
                    <div class="flex items-center space-x-2 mb-2">
                        <svg class="w-5 h-5" style="color: var(--cpsu-green);" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="mt-3">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="text" id="generatedPasswordDisplay" readonly
                                class="flex-1 px-3 py-2 rounded-lg font-mono text-lg font-bold text-center"
                                style="background-color: var(--card-bg); color: var(--cpsu-green); border: 2px solid var(--cpsu-green-light); letter-spacing: 0.1em;">
                            <button id="copyPasswordBtn" onclick="copyPassword(this)"
                                class="px-3 py-2 rounded-lg text-sm font-medium text-white transition-colors"
                                style="background: linear-gradient(135deg, var(--cpsu-green) 0%, var(--cpsu-green-light) 100%);"
                    ...
                    <p class="text-xs text-secondary mt-2">Total regenerated: <span class="font-semibold text-primary"
                            id="updatedRegenerateCount">0</span> time(s)</p>
                </div>
            </div>
            <div class="modal-footer">
                <button id="cancelRegenerateBtn" onclick="closeModal('regeneratePasswordModal')"
                ...
                <button id="regeneratePasswordBtn" onclick="confirmRegeneratePassword()"
        ...
        <div class="modal-content" style="max-width: 400px;">
            ...
            <div class="modal-body">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <p class="text-sm font-medium text-primary">Are you sure you want to delete this student account?
                        </p>
            ...
            <div class="modal-footer">
                <button onclick="closeModal('deleteAccountModal')"
                    class="px-4 py-2 text-sm font-medium rounded-lg transition-colors"
                    style="background-color: var(--bg-tertiary); color: var(--text-primary);"
                    onmouseover="this.style.backgroundColor='var(--hover-bg)'"
                    onmouseout="this.style.backgroundColor='var(--bg-tertiary)'">
